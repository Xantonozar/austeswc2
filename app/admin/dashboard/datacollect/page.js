...
import {
    ...
    FileSpreadsheet, FileJson, Image as ImageIcon, Users,
...
export default function DataCollectAdmin() {
    ...
    const getPositionCount = (position) => records.filter(r => r.position === position).length;

    const handleExportAllExcel = () => {
        const data = filteredRecords.map(mapRecordToExport);
        downloadExcel(data, `All_DataCollect_${dateStr}.xlsx`, 'DataCollect');
        setShowExportMenu(false);
    ...
    const handleExportAllJSON = () => {
        const data = filteredRecords.map(mapRecordToExport);
        downloadJSON({ records: data }, `All_DataCollect_${dateStr}.json`);
        setShowExportMenu(false);
        toast.success(`Exported ${data.length} records (JSON)`);
    };

    const handleExportByDesignationExcel = (position) => {
        const posRecords = records.filter(r => r.position === position);
        if (posRecords.length === 0) { toast.error(`No ${position} records`); return; }
        const data = posRecords.map(mapRecordToExport);
        downloadExcel(data, `${position}_DataCollect_${dateStr}.xlsx`, position);
        setShowExportMenu(false);
        toast.success(`Exported ${data.length} ${position} records (Excel)`);
    };

    const handleExportByDesignationJSON = (position) => {
        const posRecords = records.filter(r => r.position === position);
        if (posRecords.length === 0) { toast.error(`No ${position} records`); return; }
        const data = posRecords.map(mapRecordToExport);
        downloadJSON({ designation: position, exportedAt: new Date().toISOString(), totalRecords: data.length, records: data }, `${position}_DataCollect_${dateStr}.json`);
        setShowExportMenu(false);
        toast.success(`Exported ${data.length} ${position} records (JSON)`);
    };
                            ...
                            <AnimatePresence>
                                {showExportMenu && (
                                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }}
                                        className="absolute right-0 mt-2 w-72 bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden z-50 max-h-[80vh] flex flex-col">

                                        <div className="overflow-y-auto flex-1 p-2">
                                            <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 px-3 py-2">Export All (Combined)</p>
                                            <button onClick={handleExportAllExcel}
                                                ...
                                                <div className="w-8 h-8 rounded-lg bg-green-100 group-hover:bg-green-200 flex items-center justify-center">
                                                    <FileSpreadsheet className="w-4 h-4 text-green-600" />
                                                </div>
                                                <div className="flex-1">
                                                    <p className="text-sm font-bold text-slate-800">All Combined (Excel)</p>
                                                    <p className="text-[10px] text-slate-400">{filteredRecords.length} records</p>
                                                </div>
                                            </button>
                                            <button onClick={handleExportAllJSON}
                                                className="w-full flex items-center gap-3 px-3 py-2.5 text-left hover:bg-orange-50 rounded-lg transition-colors group">
                                                <div className="w-8 h-8 rounded-lg bg-orange-100 group-hover:bg-orange-200 flex items-center justify-center">
                                                    <FileJson className="w-4 h-4 text-orange-600" />
                                                </div>
                                                <div className="flex-1">
                                                    <p className="text-sm font-bold text-slate-800">All Combined (JSON)</p>
                                                    <p className="text-[10px] text-slate-400">{filteredRecords.length} records</p>
                                                </div>
                                            </button>

                                            <div className="h-px bg-slate-100 my-2" />
                                            <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 px-3 py-2">By Designation</p>

                                            {positions.map(pos => {
                                                const count = getPositionCount(pos);
                                                return (
                                                    <div key={pos} className="flex items-center gap-1 px-1 py-1 rounded-lg hover:bg-slate-50 transition-colors group">
                                                        <span className="flex-1 text-xs font-semibold text-slate-700 truncate pl-2">{pos}</span>
                                                        <span className="text-[10px] font-bold text-slate-400 mr-1 shrink-0">{count}</span>
                                                        <button onClick={() => handleExportByDesignationExcel(pos)} disabled={count === 0}
                                                            className={`p-1.5 rounded-md transition-all ${count === 0 ? 'text-slate-300 cursor-not-allowed' : 'text-green-600 hover:bg-green-100'}`} title={`Export ${pos} Excel`}>
                                                            <FileSpreadsheet className="w-3.5 h-3.5" />
                                                        </button>
                                                        <button onClick={() => handleExportByDesignationJSON(pos)} disabled={count === 0}
                                                            className={`p-1.5 rounded-md transition-all ${count === 0 ? 'text-slate-300 cursor-not-allowed' : 'text-orange-600 hover:bg-orange-100'}`} title={`Export ${pos} JSON`}>
