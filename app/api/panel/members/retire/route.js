...
export async function POST(req) {
    try {
        ...
        member.leftReason = reason || 'retired';
        await member.save();

        return NextResponse.json({ success: true, message: `${member.name} has been retired to alumni` });
    } catch (error) {
        console.error('Error retiring member:', error);
        return NextResponse.json({ error: 'Failed to retire member' }, { status: 500 });
    }
}
