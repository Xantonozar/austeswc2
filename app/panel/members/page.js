...
export default function MembersPage() {
    ...
    const MemberCard = ({ member }) => {
        ...
        return (
            <div
                ...
                className="bg-white rounded-2xl border border-slate-100 p-4 hover:shadow-lg hover:border-blue-200 transition-all cursor-pointer group relative overflow-hidden"
            >
                {/* Gradient accent top */}
                ...
                {isAlumniOrKicked && (
                    <div className="mt-3 pt-3 border-t border-slate-100">
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${member.status === 'kicked' ? 'text-red-600 bg-red-50' : 'text-blue-600 bg-blue-50'}`}>
    ...
    const DeptSection = ({ title, icon, membersList }) => {
        ...
        <div className="p-6 lg:p-8 w-full space-y-6 animate-in fade-in duration-500 pb-20">
            ...
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-400 text-sm mt-1">{filteredMembers.length} members · Click a card to manage</p>
                </div>
                {isAdmin && activeTab === 'active' && (
                    <button onClick={() => setIsAddModalOpen(true)}
                        className="bg-gradient-to-r from-blue-600 to-violet-600 text-white px-5 py-2.5 rounded-xl font-bold flex items-center gap-2 hover:from-blue-700 hover:to-violet-700 transition-all shadow-lg shadow-blue-500/20 active:scale-95">
            ...
            <div className="flex gap-2 bg-slate-100 rounded-xl p-1 w-fit">
                {[
                    ...
                    <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                        className={`px-5 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === tab.id ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                        {tab.label}
                        <span className={`ml-2 text-xs px-2 py-0.5 rounded-full ${activeTab === tab.id ? 'bg-blue-100 text-blue-600' : 'bg-slate-200 text-slate-500'}`}>{tab.count}</span>
                    </button>
                ))}
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
                <div className="relative flex-1">
                    ...
                    <input type="text" placeholder="Search members..."
                        className="w-full bg-white border border-slate-200 rounded-xl py-2.5 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent shadow-sm"
            ...
            {activeTab === 'active' ? (
                <div className="space-y-8">
                    {coreCouncil.length > 0 && <DeptSection title="Core Council" icon={<span className="w-2 h-2 rounded-full bg-gradient-to-r from-blue-500 to-violet-500"></span>} membersList={coreCouncil} />}
                    {byDept.map(dept => (
                        <DeptSection key={dept.id} title={dept.name} icon={<span className={`w-2 h-2 rounded-full ${dept.color.split(' ')[0]}`}></span>} membersList={dept.members} />
                    ))}
                </div>
            ) : (
                isLoadingAlumni ? (
                    ...
                    <div className="py-20 text-center">
                        ...
                        <p className="text-slate-400">No alumni found.</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                ...
                <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-300">
                    <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
                        ...
                        <form onSubmit={async (e) => {
                            ...
                            await addMember({
                                name: fd.get('name'), designation: fd.get('designation'), department: fd.get('department') || null,
                                rankLevel: parseInt(fd.get('rankLevel')), username: fd.get('username').toLowerCase(), password: fd.get('password'),
                                imageUrl: uploadedImageUrl || fd.get('imageUrl'),
                            });
                            setIsAddModalOpen(false); setUploadedImageUrl('');
                        }} className="p-6 space-y-4">
                            <input required name="name" placeholder="Full Name" className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
                            <input name="imageUrl" type="url" placeholder="Profile Picture URL" value={uploadedImageUrl} onChange={(e) => setUploadedImageUrl(e.target.value)}
                                className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
                            <div className="grid grid-cols-2 gap-3">
                                <select required name="designation" className="bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 text-sm focus:ring-2 focus:ring-blue-500 outline-none appearance-none">
                                    {Object.keys(ROLE_HIERARCHY).map(r => <option key={r} value={r}>{r}</option>)}
                                </select>
                                <select name="department" className="bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 text-sm focus:ring-2 focus:ring-blue-500 outline-none appearance-none">
                                    <option value="">Global / None</option>
                                    {DEPARTMENTS.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}
                                </select>
                            </div>
                            <select required name="rankLevel" className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 text-sm focus:ring-2 focus:ring-blue-500 outline-none appearance-none">
                                {Object.entries(ROLE_HIERARCHY).sort((a, b) => b[1] - a[1]).map(([r, l]) => <option key={r} value={l}>{r} (Level {l})</option>)}
                            </select>
                            <div className="grid grid-cols-2 gap-3">
                                <input required name="username" placeholder="Username" className="bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
                                <input required name="password" type="password" placeholder="Password" className="bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
                            </div>
                            <div className="flex gap-3 pt-2">
                                <button type="button" onClick={() => setIsAddModalOpen(false)} className="flex-1 py-3 rounded-xl font-bold text-slate-500 hover:bg-slate-100">Cancel</button>
                                <button type="submit" className="flex-[2] py-3 rounded-xl font-bold bg-gradient-to-r from-blue-600 to-violet-600 text-white hover:from-blue-700 hover:to-violet-700 shadow-lg shadow-blue-500/20">Create Member</button>
                            </div>
                        </form>
                    </div>
                </div>
