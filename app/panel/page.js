...
import { Users, Building2, Medal, FileText, ArrowRight, Trophy } from 'lucide-react';
import Link from 'next/link';
import { useRouter } from 'next/navigation';
import Avatar from './components/Avatar';

export default function DashboardHome() {
    const { members, currentUser, isLoggedIn, DEPARTMENTS } = useDashboard();
    ...
    // Unique departments from members (or use constant)
    const totalDepartments = new Set(displayMembers.filter(m => m.department).map(m => m.department)).size + 1; // +1 for Global

    // Sorted members for Leaderboard widget
    ...
    return (
        <div className="p-8 max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                {/* Total Members */}
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#EBF4E6] flex items-center justify-between group hover:shadow-md transition-shadow">
                    <div>
                        <p className="text-4xl font-yeseva text-[#2E5940] mb-1">{totalMembers}</p>
                        <p className="text-sm text-[#7A9080] font-medium uppercase tracking-wider">Total Members</p>
                    </div>
                    <div className="bg-[#4A7C59]/10 p-3 rounded-xl text-[#4A7C59]">
                        <Users className="w-8 h-8" />
                    </div>
                </div>

                {/* Departments */}
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#EBF4E6] flex items-center justify-between hover:shadow-md transition-shadow">
                    <div>
                        <p className="text-4xl font-yeseva text-[#2E5940] mb-1">{totalDepartments}</p>
                        <p className="text-sm text-[#7A9080] font-medium uppercase tracking-wider">Departments</p>
                    </div>
                    <div className="bg-[#4A7C59]/10 p-3 rounded-xl text-[#4A7C59]">
                ...
                {/* Top Scorer */}
                <div className="bg-[#1E3A28] rounded-2xl p-6 shadow-md border border-[#2E5940] text-white hover:shadow-lg transition-shadow relative overflow-hidden">
                    <div className="absolute top-0 right-0 w-32 h-32 bg-[#4A7C59] rounded-full opacity-20 -mr-10 -mt-10 blur-xl"></div>
                    <div className="flex items-start justify-between relative z-10">
                        <div>
                            <p className="text-2xl font-yeseva mb-1 truncate max-w-[140px]">{topScorer?.name}</p>
                            <p className="text-sm text-[#C8DDD0] font-medium mb-2 uppercase tracking-wider">Top Scorer</p>
                            <p className="text-base font-bold text-[#6BA583]">Score: {topScorer?.score}</p>
                        </div>
                        <div className="bg-[#4A7C59]/30 p-3 rounded-xl text-[#F7F3EE]">
                            <Medal className="w-7 h-7 text-yellow-500 fill-yellow-500/20" />
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#EBF4E6] flex items-center justify-between hover:shadow-md transition-shadow">
                    <div>
                        <p className="text-4xl font-yeseva text-[#2E5940] mb-1">{evalsThisMonth}</p>
                        <p className="text-sm text-[#7A9080] font-medium uppercase tracking-wider">Evals This Month</p>
                    </div>
                    <div className="bg-[#4A7C59]/10 p-3 rounded-xl text-[#4A7C59]">
                        ...
                        {allActivities.slice(0, 6).map((activity, idx) => (
                            <div key={idx} className="flex gap-4 relative">
                                ...
                                <div className="flex-1 bg-gray-50/50 p-4 rounded-xl border border-gray-100 shadow-sm relative z-10 transition-colors hover:bg-white">
                                    ...
                                    <p className="text-sm text-gray-600 italic border-l-2 border-[#6BA583] pl-3 mt-3">&qout;{activity.note}&qout;</p>
                    ...
                    <div className="p-6 flex-1 flex flex-col">
                        <div className="space-y-4 flex-1">
                            {top5.map((member, index) => (
                                <div key={member.id} className="flex items-center gap-4 group p-2 -mx-2 rounded-xl hover:bg-[#F7F3EE] transition-colors">
                                    ...
                                    <div className="font-bold text-[#2E5940] tabular-nums bg-[#EBF4E6] px-3 py-1 rounded-lg">
                                        {member.score}
                                    </div>
                                </div>
                            ))}
