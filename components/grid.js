    ...
    <div className="relative flex h-auto w-full flex-col overflow-hidden bg-background">
      ...
      <div className="whitespace-pre-wrap text-center tracking-tighter text-black dark:text-white">
        <div className="relative w-full h-screen max-h-[800px] bg-cover justify-center items-center bg-center flex flex-col md:flex-row">
          <div className="w-full ml-4 mt-8 mb-4 md:mt-1 md:ml-6 md:w-1/2 flex flex-col text-left gap-2 text-green-700 px-4">
          ...
          <div className="w-full h-full md:w-1/2 z-30 relative hidden md:block">
            {/* Only showing hero image on MD+ to match previous logic logic implied by layout, or use Image optimization */}
            <Image
              src={heroOne}
              alt="Hero Image"
              fill
              className="object-contain md:pb-20 z-30"
            />
          </div>
          {/* Mobile hero image if needed, or keeping it hidden/adjusted as per original logic which had it relative but maybe hidden by overflow or size? 
                 The original had it in a div w-full h-full md:w-1/2. 
                 On mobile, flex-col makes the first div take space. 
                 The second div with image would be below. 
                 Let's keep it visible but properly sized.
             */}
          <div className="w-full h-64 md:h-full md:w-1/2 z-30 relative md:hidden">
            <Image
              src={heroOne}
              alt="Hero Image"
              fill
