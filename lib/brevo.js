...
export const sendCompetitionRegistrationEmail = async (toEmail, toName, competitionType, extra = {}) => {
    ...
    try {
        ...
        const membersStr = members.length > 0 ? members.map(m => typeof m === 'string' ? m : m.name).filter(Boolean).join(', ') : '';
        const fee = extra.fee || null;
                        ...
                        <div class="container">
                            ...
                            <div class="content">
                                ...
                                <div class="details">
                                    ...
                                    <p><span>Participant / Team:</span><br><strong>${teamName}</strong></p>
                                    ${fee ? `<p><span>Registration Fee (Paid):</span><br><strong>BDT ${fee}</strong></p>` : ''}
                                    <p><span>Segment:</span><br><strong>${segment}</strong></p>
                                    ${membersStr ? `<p><span>Team Members:</span><br><strong>${membersStr}</strong></p>` : ''}
                                </div>
...
export const sendSelectionEmail = async (toEmail, toName, competitionType, extra = {}) => {
    ...
    try {
        const segment = formatSegment(competitionType);
        const teamName = extra.teamName || toName || 'Participant';
        const isEcoFrame = competitionType === 'eco-frame';
        let subject, htmlContent;

        if (isEcoFrame) {
            subject = `Congratulations! You've Been Shortlisted — Eco Champions 4.0 | ${segment}`;
            htmlContent = `
                <html>
                    <head>
                        <style>
                    ...
                    <body>
                        <div class="container">
                            <div class="header">
                                ...
                                <p style="margin:6px 0 0; font-size: 13px; opacity: 0.95;">You've been shortlisted for ${segment}</p>
                            </div>
                            <div class="content">
                                <p style="margin-top:0;">Dear ${toName || teamName},</p>
                                <p>Greetings from <strong>AUST Environmental & Social Welfare Club (AUSTESWC)</strong>!</p>
                                <p>Congratulations! We are pleased to inform you that your entry for <strong>${segment}</strong> under <strong>Eco Champions 4.0</strong> has been <strong>shortlisted</strong> based on the evaluation of Round 1.</p>
                                <div class="details">
                                    <p style="font-weight:800; color:#1B4B43; margin-bottom:10px; font-size:14px;">Shortlist Details:</p>
                                    <p><span>Participant / Team:</span><br><strong>${teamName}</strong></p>
                                    <p><span>Segment:</span><br><strong>${segment}</strong></p>
                                </div>
                                <div class="callout">
                                    <p style="margin:0; font-size:14px;">You are now <strong>invited to the Eco Champions 4.0 main event</strong>, where the final results will be announced.</p>
                                    <p style="margin:10px 0 0; font-size:13px; color:#9a3412;"><strong>The event date and time will be communicated to you shortly via email.</strong> Please keep an eye on your inbox.</p>
                                </div>
                                <div class="details">
                                    <p style="font-weight:800; color:#1B4B43; margin-bottom:10px; font-size:14px;">Facebook Showcase & Evaluation:</p>
                                    <p style="margin:6px 0; font-size:14px;">Your shortlisted entry will be showcased on the official <strong>AUSTESWC Facebook page</strong> as part of the Eco Champions 4.0 public exhibition. The audience interaction — including <strong>likes, comments, and shares</strong> — on your post will play a <strong>significant role in the final evaluation and selection process</strong>.</p>
                                    <p style="margin:10px 0 0; font-size:13px; color:#64748b;">We encourage you to share your post within your network to maximize engagement, as it will directly contribute to your overall scoring.</p>
                                </div>
                                <p>No additional payment is required. You are all set for the main event!</p>
            ...
            subject = `Congratulations! You're Eligible for Round 2 — Eco Champions 4.0 | ${segment}`;
            htmlContent = `
                <html>
                    <head>
                        <style>
...
export const sendPaymentConfirmationEmail = async (toEmail, toName, competitionType, extra = {}) => {
    if (!process.env.BREVO_API_KEY) {
    ...
    try {
        const segment = formatSegment(competitionType);
        const amount = extra.amount || '149';
        const method = extra.method || 'bKash';
        const trxId = extra.trxId || '';
        const subject = `Payment Received — Eco Champions 4.0 | ${segment}`;
        const htmlContent = `
                <html>
                    <head>
                        <style>
                    ...
                    <body>
                        <div class="container">
                            <div class="header">
                                <img src="https://austeswc.org/eswclogo.png" alt="AUSTESWC Logo" style="width: 72px; height: auto; margin-bottom: 12px; background: white; border-radius: 50%; padding: 6px;">
                                <h2 style="margin:0; font-size: 18px; letter-spacing: 0.5px;">Payment Received ✅</h2>
                                <p style="margin:6px 0 0; font-size: 13px; opacity: 0.9;">Eco Champions 4.0</p>
                            </div>
                            <div class="content">
                                <p style="margin-top:0;">Dear ${toName || 'Participant'},</p>
                                <p>Thank you! We have successfully received and verified your registration payment for the <strong>${segment}</strong> segment of <strong>Eco Champions 4.0</strong>.</p>
                                <div class="details">
                                    <p style="font-weight:800; color:#1B4B43; margin-bottom:10px; font-size:14px;">Payment Details:</p>
                                    <p><span>Segment:</span><br><strong>${segment}</strong></p>
                                    <p><span>Amount Paid:</span><br><strong>BDT ${amount}</strong></p>
                                    <p><span>Method:</span><br><strong>${method}</strong></p>
                                    ${trxId ? `<p><span>Transaction ID:</span><br><strong>${trxId}</strong></p>` : ''}
                                </div>
                                <p>Your registration is now <strong>complete</strong>. Please keep this email as proof of payment. Further updates regarding evaluation, the Facebook showcase, and the on-campus exhibition will be shared with you via email and our Facebook page (<a href="https://www.facebook.com/aust.eswc">AUSTESWC</a>).</p>
        ...
        if (!response.ok) {
            const errData = await response.text();
            console.error("Brevo Payment Confirmation API Error:", response.status, errData);
            throw new Error(`Brevo HTTP error: ${response.status}`);
        }
        console.log('Brevo payment confirmation email sent to', toEmail);
        return true;
    } catch (error) {
...
export const sendEcoFramePhotoPostedEmail = async (toEmail, toName) => {
    if (!process.env.BREVO_API_KEY) {
        console.log(`[Mock Email] Eco Frame photo posted email would have been sent to ${toEmail}`);
        return true;
    }
    try {
        ...
        console.log('Brevo eco-frame photo posted email sent to', toEmail);
        return true;
    } catch (error) {
        console.error('Error sending eco-frame photo posted email:', error.message);
        return false;
    }
};
